...
public class WordSearch {
    ...
    public boolean dfs(char[][] board, int i, int j, String word,boolean[][] visited, int index) {
        if(i >= board.length|| j>= board[0].length|| i<0||j<0 || visited[i][j]|| index>= word.length()) {
            return false;
        }
        ...
        boolean h1 = dfs(board, i + 1, j, word, visited, index + 1);
        boolean h2 = dfs(board, i - 1, j, word, visited, index + 1);
        boolean h3 = dfs(board, i, j + 1, word, visited, index + 1);
        boolean h4 = dfs(board, i, j - 1, word, visited, index + 1);
        if( h1 || h2 || h3 || h4) {
            return true;
        }
        visited[i][j] = false;
